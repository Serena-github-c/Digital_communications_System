...
%% Downsampling to simulate lower sampling rates
x_2000 = x;                         % original sampled at 2000 Hz
x_1000 = x(1:2:end);                % sampled at 1000 Hz
x_500  = x(1:4:end);                % sampled at 500 Hz
...
% Plot
figure('Name', 'Effect of Sampling Rate on Signal', 'NumberTitle', 'off');

subplot(3,1,1);
stem(t_2000(1:200), x_2000(1:200), 'filled');
xlabel('Time (s)'); ylabel('Amplitude');
title('Sampling at Fs = 2000 Hz (Original)');
grid on;

subplot(3,1,2);
stem(t_1000(1:100), x_1000(1:100), 'filled');
xlabel('Time (s)'); ylabel('Amplitude');
title('Sampling at Fs = 1000 Hz');
grid on;

subplot(3,1,3);
stem(t_500(1:50), x_500(1:50), 'filled');
xlabel('Time (s)'); ylabel('Amplitude');
title('Sampling at Fs = 500 Hz');
grid on;
